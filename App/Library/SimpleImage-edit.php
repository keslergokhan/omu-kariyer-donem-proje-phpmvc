...
//
//  Source: https://github.com/claviska/SimpleImage_edit
...
class SimpleImage_edit {
    ...
    //
    // Creates a new SimpleImage_edit object.
    //
    ...
    //
    // Returns a SimpleImage_edit object.
    //
    public function fromDataUri($uri) {
        ...
        if(!$this->image) {
            throw new \Exception("Geçersiz dosya formatı !.", self::ERR_INVALID_IMAGE);
        }
    ...
    public function fromFile($file) {
        ...
        $handle = @fopen($file, 'r');
        if($handle === false) {
            throw new \Exception("Dosya bulunamadı !: $file", self::ERR_FILE_NOT_FOUND);
        }
        fclose($handle);
        ...
        if($info === false) {
            throw new \Exception("Geçersiz dosya : $file", self::ERR_INVALID_IMAGE);
        }
    ...
    //
    //  $overlay* (string|SimpleImage_edit) - The image to overlay. This can be a filename, a data URI, or
    //    a SimpleImage_edit object.
    ...
    public function overlay($overlay, $anchor = 'center', $opacity = 1, $xOffset = 0, $yOffset = 0) {
        // Load overlay image
        if(!($overlay instanceof SimpleImage_edit)) {
            $overlay = new SimpleImage_edit($overlay);
        }
    ...
    public function text($text, $options, &$boundary = null) {
        ...
        //
        // See: https://github.com/claviska/SimpleImage_edit/issues/165
        //
